import {lemoToMo, moToLemo, base64Encode, base64Decode} from 'lemo-utils'
...
export function parseDeepLink(uri) {
    const uriContent = parseUri(uri)

    let data
    switch (uriContent.type) {
        case DeepLinkType.PAY:
            data = parsePayDeepLink(uriContent)
            break
        case DeepLinkType.SIGN:
            data = parseSignDeepLink(uriContent)
            break
        default:
            throw errors.InvalidDeepLinkType(uriContent.type)
    }
    return {
        data,
        deepLinkType: uriContent.type,
    }
}
...
export function createPayDeepLink(txConfig) {
    ...
    if (notEmpty(txConfig.expirationTime)) {
        // 传出的截止时间是秒
        newTxConfig.e = txConfig.expirationTime
    }
    if (notEmpty(txConfig.message)) {
        // 加密成safe url base64
        newTxConfig.m = base64Encode(txConfig.message, true)
    }
...
export function parsePayDeepLink(uriContent) {
    ...
    if (notEmpty(uriContent.e)) {
        // 传入的截止时间单位是秒
        txConfig.expirationTime = parseInt(uriContent.e, 10)
    }
    if (notEmpty(uriContent.m)) {
        txConfig.message = base64Decode(uriContent.m)
    }
    if (uriContent.s) {
...
export function createSignDeepLink(config) {
    ...
    if (notEmpty(config.message)) {
        // 加密成safe url base64
        newConfig.m = base64Encode(config.message, true)
    }
    if (notEmpty(config.data)) {
        // 加密成safe url base64
        newConfig.d = base64Encode(config.data, true, true)
    }
...
export function parseSignDeepLink(uriContent) {
    ...
    const config = {}
    if (notEmpty(uriContent.m)) {
        config.message = base64Decode(uriContent.m)
    }
    if (notEmpty(uriContent.d)) {
        config.data = base64Decode(uriContent.d, true)
    }
...
function parseUri(uri) {
    ...
    termArr.forEach(param => {
        ...
        if (uriContent[key] === undefined) {
            uriContent[key] = value
        } else {  // 表示这个键对应的值是一个数组
            uriContent[key] = [value].concat(uriContent[key])
        }
    })
    uriContent.type = linkConfig.type
    return uriContent
}
