...
export function toRaw(data, fieldName, length) {
    data = lemoUtils.toBuffer(data)
    if (length) {
        if (data.length > length) {
            throw new Error(errors.TXFieldToLong(fieldName, length))
        }
        data = lemoUtils.setBufferLength(data, length, false)
    } else {
        data = lemoUtils.bufferTrimLeft(data)
    }
    return data
}
...
export function verifyCreateAssetInfo(config) {
    ...
    if (config.decimal > MAX_DECIMAL_DIGITS) {
        throw new Error(errors.DecimalDigitsError())
    }
...
export function verifyBoxTXInfo(subTxList) {
    ...
    subTxList.forEach(item => {
        ...
        if (obj.data) {
            obj.data = JSON.parse(lemoUtils.decodeUtf8Hex(obj.data))
        }
        verifyTxConfig(obj)
    ...
    for (let i = 0; i < types.length; i++) {
        ...
        if (typeStr === types[i]) {
            ...
            if (typeStr === 'string' && isNumber) {
                const isHex = lemoUtils.has0xPrefix(data)
...
function checkRange(obj, fieldName, from, to) {
    ...
    if (typeof data === 'string') {
        data = parseInt(data, lemoUtils.has0xPrefix(data) ? 16 : 10)
    }
...
function checkBytesLength(str, strName, maxBytesLength) {
    ...
    dataLen = Math.ceil(dataLen / 2 - (lemoUtils.has0xPrefix(str) ? 1 : 0))
...
function checkAddress(address) {
    const errMsg = lemoUtils.verifyAddress(address)
    if (errMsg) {
