...
export default class Tx {
    ...
    normalize(txConfig) {
        ...
        if (txConfig.data && typeof txConfig.data === 'object') {
            this.data = lemoUtils.toHexStr(Buffer.from(JSON.stringify(txConfig.data)))
        } else {
            this.data = lemoUtils.toHexStr(txConfig.data)
        }
        // message string
        this.message = (txConfig.message || '').toString()
        // sigs Array
        if (txConfig.sigs && Array.isArray(txConfig.sigs)) {
            this.sigs = txConfig.sigs.map(lemoUtils.toHexStr)
        } else {
            this.sigs = []
        }
        // gasPayerSig Array
        if (txConfig.gasPayerSigs && Array.isArray(txConfig.gasPayerSigs)) {
            this.gasPayerSigs = txConfig.gasPayerSigs.map(lemoUtils.toHexStr)
        } else {
            this.gasPayerSigs = []
        }
        // from string
    ...
    serialize() {
        const raw = [
            ...
            toRaw(lemoUtils.decodeAddress(this.from), 'from', TX_ADDRESS_LENGTH),
            this.gasPayer ? toRaw(lemoUtils.decodeAddress(this.gasPayer), 'gasPayer', TX_ADDRESS_LENGTH) : '',
            this.to ? toRaw(lemoUtils.decodeAddress(this.to), 'to', TX_ADDRESS_LENGTH) : '',
            toRaw(this.toName, 'toName'),
            toRaw(this.gasPrice, 'gasPrice'),
            toRaw(this.gasLimit, 'gasLimit'),
            toRaw(this.amount, 'amount'),
            this.type === TxType.BOX_TX ? arrayToRaw(this.getHashData(), 'data') : toRaw(this.data, 'data'),
        ...
        return lemoUtils.rlpEncode(raw)
    }

    getHashData() {
        const data = JSON.parse(lemoUtils.toBuffer(this.data).toString())
     ...
     */
    hash() {
        const hashBuffer = lemoUtils.sha3(this.serialize())
