...
export default class Signer {
    ...
    hashForSign(tx) {
        const raw = [
            ...
            tx.type === TxType.BOX_TX ? toRaw(this.getSignHashData(tx), 'data') : toRaw(tx.data, 'data'),
    ...
    getSignHashData(tx) {
        ...
        const result = []
        data.subTxList.forEach(item => {
            const TX = new Tx(item)
            result.push(sha3(TX.serialize()))
        })
        return sha3(rlpEncode(result))
    }
}
